    ...
    <tr className="border-2 hover:bg-slate-100">
      ...
      <td className="border-2">
        ...
        <a
          ...
          href={`/leery/admin/dashboard/track-user/${data.email}`}
          target="_blank">
          {data?.email}
        </a>
      </td>
      ...
      <td className="border-2">
        <p className="bg-green-50 border border-green-500  text-center font-semibold rounded-full">
          {data?.resellerStatus}
        </p>
      </td>
      <td className="border-2">
        <p className="badge-success rounded-badge text-center font-bold text-white">
          {data?.balance}
        </p>
      </td>
      <th className="flex justify-center items-center gap-2">
        <button
          ...
          className={`btn btn-xs tooltip text-white ${
            data.status === true ? "btn-success" : "bg-slate-300"
          }`}
          data-tip="Status Change">
          <FaPowerOff></FaPowerOff>{" "}
        </button>
        <Link
          to={`/leery/admin/dashboard/track-user/${data.email}`}
        ...
        <button
          ...
          data-tip={"Pass Change"}>
          <MdPassword></MdPassword>
        </button>
        <button
          onClick={() => showModal1(data)}
          className="btn btn-xs btn-neutral text-white tooltip"
          data-tip={"Bonus"}>
          <FaCoins></FaCoins>
        </button>
        <button
          onClick={() => HandleDelete(axiosSecure, refetch, data._id, "user")}
