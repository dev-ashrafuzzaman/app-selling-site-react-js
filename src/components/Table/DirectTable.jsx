...
const DirectTable = ({ data, sl, refetch, axiosSecure }) => {
    return (
        <tr className='border-2 hover:bg-slate-100'>
            ...
            <th className="flex justify-center items-center gap-2">
                <button onClick={() => HandleStatusChange(axiosSecure, refetch, data._id, 'direct', !data.status)} className={`btn btn-xs tooltip text-white ${data.status === true ? 'btn-success' : 'bg-slate-300'}`} data-tip="Status Change"><FaPowerOff></FaPowerOff> </button>
                <button onClick={() => HandleDelete(axiosSecure, refetch, data._id, 'direct')} className="btn btn-xs btn-error text-white tooltip" data-tip="Delete" ><FaTrash></FaTrash></button>
            </th>
        </tr>
    );
};
