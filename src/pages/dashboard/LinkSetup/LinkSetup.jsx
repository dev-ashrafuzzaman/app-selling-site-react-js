...
const LinkSetup = () => {
    ...
    const [dailyCommission, setDailyCommission] = useState(isGlobalData?.result?.dailyCommission);
    const [referBonus, setReferBonus] = useState(isGlobalData?.result?.referBonus);
    ...
    const [telegramChannel, setTelegramChannel] = useState(isGlobalData?.result?.telegramChannel);
    const [telegramGroup, setTelegramGroup] = useState(isGlobalData?.result?.telegramGroup);
            ...
            <div className="bg-slate-100 max-w-screen-lg mx-auto p-6 my-10 rounded-lg">
                ...
                <div className="w-full">
                    ...
                    <div className="join w-full">
                        <div className="w-full">
                            <div>
                                <input defaultValue={referBonus} onChange={handleChangeReferBonus} className="input w-full input-bordered join-item" placeholder="Daily Commission" />
                            </div>
                        </div>
                        <button onClick={() => handleGlobalUpdate(setLoading, refBonusData, axiosSecure, isGlobalData?.result?._id, refetch)} className="btn join-item btn-success text-white">Update</button>
                    </div>
                </div>
                ...
                <div className="border-2 border-dashed p-2 rounded-lg my-4">
                    ...
                    <div className="md:flex items-end space-y-2 gap-4">
                        <div>
                            ...
                            <input defaultValue={withdrawValues?.minAmount} onChange={e => setWithdrawValues({ ...withdrawValues, minAmount: e.target.value })} className="input w-full input-bordered join-item" placeholder="Min Amount" />
                        </div>
                        <div>
                            <div className="label">
                                <span className="label-text">Max amount</span>
                            </div>
                            <input defaultValue={withdrawValues?.maxAmount} onChange={e => setWithdrawValues({ ...withdrawValues, maxAmount: e.target.value })} className="input w-full input-bordered join-item" placeholder="Max Amount" />
                        </div>
                        <div>
                            <div className="label">
                                <span className="label-text">Cash out charge</span>
                            </div>
                            <input defaultValue={withdrawValues?.outAmount} onChange={e => setWithdrawValues({ ...withdrawValues, outAmount: e.target.value })} className="input w-full input-bordered join-item" placeholder="Out Charge" />
                        </div>
                        <div>
                            <div className="label">
                                <span className="label-text">Mini refer</span>
                            </div>
                            <input defaultValue={withdrawValues?.minRef} onChange={e => setWithdrawValues({ ...withdrawValues, minRef: e.target.value })} className="input w-full input-bordered join-item" placeholder="Min Refer" />
                        </div>
                        <div>
                            <div className="label">
                                <span className="label-text">Withdraw time</span>
                            </div>
                            <input defaultValue={withdrawValues?.withTime} onChange={e => setWithdrawValues({ ...withdrawValues, withTime: e.target.value })} className="input w-full input-bordered join-item" placeholder="10am to 5pm" />
                        </div>
                        <button onClick={() => handleGlobalUpdate(setLoading, withdrawRuleData, axiosSecure, isGlobalData?.result?._id, refetch)} className="btn  btn-success text-white">Update</button>
                    </div>
                </div>
                ...
                <div className="w-full">
                    ...
                    <div className="join w-full">
                        <div className="w-full">
                            <div>
                                <input defaultValue={telegramChannel} onChange={handleteleChanal} className="input w-full input-bordered join-item" placeholder="Link" />
                            </div>
                        </div>
                        <button onClick={() => handleGlobalUpdate(setLoading, teleChanalData, axiosSecure, isGlobalData?.result?._id, refetch)} className="btn join-item btn-success text-white">Update</button>
                    </div>
                </div>
                ...
                <div className="w-full">
                    <div className="label">
                        <span className="label-text">Telegram Group</span>
                    </div>
                    <div className="join w-full">
                        <div className="w-full">
                            <div>
                                <input defaultValue={telegramGroup} onChange={handleteleGroup} className="input w-full input-bordered join-item" placeholder="Link" />
                            </div>
                        </div>
                        <button onClick={() => handleGlobalUpdate(setLoading, teleGroupData, axiosSecure, isGlobalData?.result?._id, refetch)} className="btn join-item btn-success text-white">Update</button>
                    </div>
                </div>
                ...
                <div className="w-full">
                    ...
                    <div className="md:join w-full">
                        <div className="w-full">
                            <div>
                                <textarea defaultValue={topNews} onChange={handleTopNews} className="textarea textarea-bordered textarea-lg w-full join-item" placeholder="Link" />
                            </div>
                        </div>
                        <button onClick={() => handleGlobalUpdate(setLoading, topNewsData, axiosSecure, isGlobalData?.result?._id, refetch)} className="btn join-item btn-success text-white">Update</button>
                    </div>
                </div>
