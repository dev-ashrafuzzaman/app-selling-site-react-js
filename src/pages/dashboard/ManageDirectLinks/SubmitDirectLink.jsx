...
import useDirectSubmit from "../../../hooks/useDirectSubmit";
import DirectSubmitTable from "../../../components/Table/DirectSubmitTable";
import { HandleDirectSubmitStatus } from "../../../utils/WebUsers/HandleDirectSubmitStatus";

const SubmitDirectLink = () => {
    const [axiosSecure] = useAxiosSecure();
    const [searchQuery, setSearchQuery] = useState('');
    const [currentPage, setCurrentPage] = useState(1);
    const [pageSize, setPageSize] = useState(10);
    const { isDirectSubmit, refetch } = useDirectSubmit(currentPage, pageSize, searchQuery);
    ...
    useEffect(() => {
        if (isDirectSubmit) {
            setTotalCount(isDirectSubmit?.totalCount);
        }
    }, [isDirectSubmit]);
    ...
    return (
        <>
            <div className="px-8">
                <SectionTitle heading={'Manage Direct Submit'}></SectionTitle>
                ...
                <div className="w-full my-4 drop-shadow-xl bg-white rounded-2xl md:p-10 p-4">
                    ...
                    <div className="overflow-x-auto bg-white p-6 rounded-2xl">
                        <table className="table">
                            ...
                            <tbody>
                                {
                                    isDirectSubmit?.data?.map((withdraw, index) => <DirectSubmitTable
                                        key={withdraw._id}
                                        data={withdraw}
                                        sl={index}
                                        refetch={refetch}
                ...
                <dialog id="withdrawStatus" ref={modalRef} className="modal">
                    <div className="modal-box">
                        ...
                        <button onClick={() => HandleDirectSubmitStatus(axiosSecure, refetch, modalData, statusValue, modalRef)} className='btn btn-error text-white my-2 w-full'>Status Change</button>
                    </div>
                </dialog>
            </div>
        </>
...
export default SubmitDirectLink;