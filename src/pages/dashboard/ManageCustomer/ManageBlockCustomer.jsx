...
const ManageBlockCustomer = () => {
    const [axiosSecure] = useAxiosSecure();
    const [searchQuery, setSearchQuery] = useState('');
    const [currentPage, setCurrentPage] = useState(1);
    const [pageSize, setPageSize] = useState(10);
    const { isBlockCustomer, refetch, isBlockCustomerLoading } = useBlockCustomer(currentPage, pageSize, searchQuery);
    ...
    useEffect(() => {
        if (isBlockCustomer) {
            setTotalCount(isBlockCustomer?.totalCount);
        }
    }, [isBlockCustomer]);
    ...
    return (
        <div className='md:px-8'>
            <SectionTitle heading={'Manage Block Users'}></SectionTitle>
            ...
            <div className='hidden md:block'>
                <div className="overflow-x-auto bg-white p-6 rounded-2xl">
                    <table className="table">
                        ...
                        <tbody>
                            {
                                isBlockCustomerLoading && <TableLoad></TableLoad>
                            }
                            {
                                isBlockCustomer?.data?.map((customer, index) => <CustomerTable
                                    key={customer._id}
                                    data={customer}
                                    sl={index}
                                    refetch={refetch}
            ...
            {/* <div className='block md:hidden'>
                ...
                {
                    isBlockCustomer?.data?.map((customer, index) => <CustomerMobileView
                        key={customer._id}
                        data={customer}
                        sl={index}
                        refetch={refetch}
            ...
            <dialog id="my_modal_2" ref={modalRef1} className="modal ">
                <div className="modal-box">
                    ...
                    <label className="input input-bordered flex items-center gap-2">
                        ...
                        <input type="number" required className="grow" defaultValue={0}
...
export default ManageBlockCustomer;